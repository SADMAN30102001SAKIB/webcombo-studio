  ...
  switch (step) {
    ...
    case "reverb_completed":
      ...
      steps[3].classList.add(
        step === "reverb_completed" ? "completed" : "active",
      );
      break;
  }
}

export function displayError(errorElements, message, errorType, screenshotUrl) {
  const {
    title,
    text,
    recommendation,
    networkRecommendation,
    reverbRecommendation,
    screenshotCard,
    screenshotImg,
    screenshotLink,
  } = errorElements;
  ...
  if (
    ...
    errorType === "REVERB_ERROR" ||
    /slowedreverb|reverb|synthesis/i.test(message)
  ) {
