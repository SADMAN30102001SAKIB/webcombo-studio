...
export async function applyReverb(c) {
  ...
  if (!stemPath || !fs.existsSync(stemPath)) {
    return c.json({ error: 'Stem audio file not available. Please extract stems first.' }, 400);
  }
  ...
  (async () => {
    try {
      const options = {
        slowByPitch: body.slowByPitch !== undefined ? Boolean(body.slowByPitch) : true,
        speed: body.speed !== undefined ? Number(body.speed) : (body.slowByPitch ? -2 : 87),
  ...
  return c.json({ success: true, message: 'Reverb processing started.', jobId });
}
...
function resolveTargetAudioPath(job, type) {
  ...
  if (type === 'reverb') {
    return job.reverbPath && fs.existsSync(job.reverbPath) ? job.reverbPath : null;
  }
...
export async function downloadAudio(c) {
  ...
  if (type === 'reverb' || (!type && job.reverbPath === targetPath)) {
    setTimeout(async () => {
      try {
        ...
        if (job.stemPath && fs.existsSync(job.stemPath)) {
          await fs.promises.unlink(job.stemPath);
        }
      } catch {}
    }, 10000);
  }
