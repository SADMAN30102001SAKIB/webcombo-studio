...
export async function applySlowedReverb(
  ...
  const slowByPitch = options.slowByPitch !== undefined ? Boolean(options.slowByPitch) : true;
  const speed = options.speed !== undefined ? Number(options.speed) : (slowByPitch ? -2 : 87);
  const reverbMix = options.reverbMix !== undefined ? Number(options.reverbMix) : 35;
  ...
  try {
    ...
    while (!isCompleted && Date.now() - pollStart < 60000) {
      const currentStatus = await page.evaluate(() => {
        ...
        return {
          ...
          hasMp3: Boolean(downloadLink && downloadLink.href && downloadLink.href.startsWith('blob:'))
        };
      });
      ...
      if (currentStatus.statusText.includes('Error')) {
        throw new Error(`SlowedReverb processing failed: ${currentStatus.statusText}`);
      }
    ...
    if (!isCompleted) {
      throw new Error('SlowedReverb processing timed out after 60 seconds. Please try again.');
    }
    ...
    const stemBaseName = path.basename(resolvedStemPath, path.extname(resolvedStemPath));
    ...
    if (page) {
      try {
        const errorScreenshot = path.join(config.downloadDir, 'slowed-error.png');
